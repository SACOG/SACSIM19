...
class BCP():
    ...
    def add_quotes_to_tstamps(self, in_file, tstamp_cols, re_dt_format=None):
        ...
        arcpy.AddMessage(f"\tquoting timestamp cols {tstamp_cols} so it can be read into SQL Server...")
        import csv
        ...
        try:
            ...
            with open(in_file, 'r') as f_in:
                ...
                for i, row in enumerate(reader):
                    ...
                    if i % 1000000 == 0: arcpy.AddMessage(f"{i} datetime rows pre-processed...")
    ...
    def create_sql_table_from_file(self, file_in, str_create_table_sql, tbl_name,
        ...
        if file_format not in self.accepted_file_types:
            arcpy.AddMessage(f"{file_format} files not presently accepted by this loader. Exiting...")
            sys.exit()
        ...
        with pyodbc.connect(self.str_conn_info, autocommit=True) as conn:
            ...
            if tbl_name in tables:
                if overwrite:
                    arcpy.AddMessage(f"{tbl_name} already exists. Will be overwritten...")
                    drop_tbl_sql = "DROP TABLE {};".format(tbl_name)
                    sql_cur.execute(drop_tbl_sql)
                else:
                    arcpy.AddMessage(f"{tbl_name} already exists. Exiting script...")
                    sys.exit()
            
            # create table that data will load to
            arcpy.AddMessage(f"creating table {tbl_name}...")
        ...
        # run bcp command
        arcpy.AddMessage(f"loading data from {file_in} into {tbl_name}...")
        try:
            subprocess.check_output(bcp_new_tbl_from_file)
            
            # import pdb; pdb.set_trace()
            if dt_cols:
                arcpy.AddMessage("loading from staging table into final table for conversion to tstamp...")
            ...
            arcpy.AddMessage(("Successfully loaded table in {}mins!\n".format(elapsed_time)))
        except:
            arcpy.AddMessage("BCP load fail. Things to try or check:\n" \
