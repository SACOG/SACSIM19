...
INSERT INTO #rad_pop_aggreg
	SELECT
		...
		SUM(CASE
			...
			END) AS emp_rad
	FROM {2} tr --taz-rad lookup table
		LEFT JOIN {3} hh --raw hh table
			ON tr.TAZ = hh.hhtaz
		LEFT JOIN {0} p --raw scenario parcel file
			ON tr.TAZ = p.taz_p
			AND p.parcelid = hh.hhparcel
	GROUP BY tr.RAD
...
CREATE TABLE #parcel_pop (
	parcelid INT,
	pcl_pop FLOAT,
	emptot_p FLOAT,
	tot_user FLOAT --total jobs + total hh pop on parcel
	)

INSERT INTO #parcel_pop
	SELECT
		p.parcelid,
		SUM(CASE
			WHEN hh.hhsize IS NULL THEN 0 ELSE hh.hhsize END) AS pcl_pop,
		p.emptot_p,
		p.emptot_p 
			+ SUM(CASE WHEN hh.hhsize IS NULL THEN 0 ELSE hh.hhsize END) 
			AS tot_user
	FROM {0} p --raw scenario parcel file
		LEFT JOIN {3} hh --raw hh file
			ON p.parcelid = hh.hhparcel
	GROUP BY p.parcelid, p.emptot_p
...
INSERT INTO #parcel_ixxi_data
	SELECT
		...
		CASE WHEN ppop.tot_user > 0 THEN p.emptot_p*rir.NRES_VT_RATE ELSE 0 END AS IX_VT_NRES,
		CASE WHEN ppop.tot_user > 0 THEN p.emptot_p*rir.NRES_VMT_RATE ELSE 0 END AS IX_VMT_NRES,
		CASE WHEN ppop.tot_user > 0 THEN p.emptot_p*rir.NRES_CVMT_RATE ELSE 0 END AS IX_CVMT_NRES,
		CASE WHEN ppop.tot_user > 0 THEN p.emptot_p*rir.NRES_VHT_RATE ELSE 0 END AS IX_VHT_NRES
	FROM {2} tr --tazrad lookup table
		...
		JOIN {0} p --raw scen parcel table; do inner join because we don't want RADs/TAZs without parcels returned
			ON tr.TAZ = p.taz_p
		LEFT JOIN #parcel_pop ppop --left join so all parcels included
