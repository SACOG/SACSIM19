#--------------------------------
# Name:make_comb_ilut_2005.py
# Purpose: combines ILUT tables for ILUT output table, but uses 2005 inputs
#          Distinct because it cannot use the 'master' parcel table, pop table, or ETO tables
#           Scripts have been modified to enter dummy variables or omit need for these tables.
# Author: Darren Conly
# Last Updated: <date>
# Updated by: <name>
# Copyright:   (c) SACOG
...
def make_tables(create_triptour_table = False,
    ...
    pop_table = "DUMMY_TABLE" #input("Copy/paste population table name: ")
    sc_yr = input("Enter scenario year: ")
    sc_code = input("Enter scenario number: ")
    av_tnc_type = '1' #input("Enter '1' ({}), '2' ({}), or '3' ({}): " \ .format(avmode_dict[1][0],avmode_dict[2][0],avmode_dict[3][0]))
    ...
    if create_person_table:
        person_params = [pop_table, raw_person, raw_parcel, person_outtbl, raw_hh]
...
if __name__ == '__main__':
    ...
    sql_dir = r"Q:\SACSIM19\Integration Data Summary\SACSIM19 Scripts\SQL\Python SQL\Year 2005"
    ...
    person_sql = "theme_person.sql" #edited SQL script to eliminate POP table
    hh_sql_yesAV = "theme_hh_yesAV.sql" #no edits to query for 2005
    hh_sql_noAV = "theme_hh_noAV.sql" #edited SQL script to eliminate POP table
    triptour_sql_noAV = "theme_triptour_VMTConstants.sql" #no edits to query for 2005
    triptour_sql_yesAV = "theme_triptour_VMTConstants.sql" #no edits to query for 2005
    
    cvixxi_sql = "theme_cveh_ixxi.sql" #no edits to query for 2005
    
    mix_density_sql1 = "mix_density_pt1.sql" #no edits to query for 2005
    mix_density_sql2 = "mix_density_pt2.sql" #no edits to query for 2005
    
    comb_sql = "ILUT_combine_tables.sql" #edited SQL script to eliminate ETO and master parcel table
    
    #Enter and confirm the envision tomorrow parcel table to use
    #NOT APPLICABLE FOR 2005
#    while True:
...
#            continue
    ...
    #if correct_env_tmrw_table:
    envision_tomorrow_tbl = "DUMMY_TABLE"
