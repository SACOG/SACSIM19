; DKS 2019/03/06 revise vdfs, linkclass, pathload layout, capclass 16
; DKS revised 2019/03/04 with 4 iterations feedback, cleaned up FB spec,
; SACOG 2019/02/08 received by DKS 2019/03/04
...
endrun
	...
	; change to *** 5 *** loops with 100% sample each; use tightest relative gap convergence for third iteration.


	loop iteration=1,4             ;endloop is at the end of the script line 3966
	    iter.i = iteration
    ...
    *echo Begin highway assignments Iter @iter.i@>timelog.beginhwyassign.iter@iter.i@.txt
	...
	loop toop = 1,5    ;toll_loop = toop
      ...
      endrun
		...
		COMBINE=EQUI,MAXITERS=300,RELATIVEGAP=@iter.relgap@,gap=0,raad=0,aad=0;rmse=0.0001
        ...
        ;10/5/2018 - for AV testing, freeways increased +20%, all other arterials +10%
          ...
          SPDCAP CAPACITY[11]=0,1500,0,0,0,1800,0,0,0                              ;special classes: 12=hi-cap river crossing;16=hi-cap ramp(*DKS*) | SPDCAP CAPACITY[11]=1800,1500,0,0,0,2000,0,0,0
		...
		;   Revised (suggested DKS 3/6/2019 for gradual arterial delay, prevent negative delay)
		
		  TC[1]=T0*min((0.9-6.0*(1-0.88*(V/C))+sqrt(36.0*(1-0.88*(V/C))*(1-0.88*(V/C))+1.21)),11.0+0.000708*(V/C))+lw.ramp*min((-0.029+sqrt(324.0*(1-1.8*(v/c))*(1-1.8*(v/c))+1.06)-18.0*(1-1.8*(v/c))),15)
		          ;highways change alpha to 5
		  TC[2]=T0*min((0.875-5.0*(1-0.92*(V/C))+sqrt(25.0*(1- 0.92*(V/C))*(1-0.92*(V/C))+1.2657)),11.0+0.000615*(V/C))+lw.ramp*min((-0.029+sqrt(324.0*(1-1.8*(v/c))*(1-1.8*(v/c))+1.06)-18.0*(1-1.8*(v/c))),15)
		          ;arterials change alpha to 4
		  TC[3]=T0*min((0.8334-4.0*(1-0.89*(V/C))+sqrt(16.0*(1-0.89*(V/C))*(1-0.89*(V/C))+1.3611)),7.0+0.000185*(V/C))+lw.ramp*min((-0.029+sqrt(324.0*(1-1.8*(v/c))*(1-1.8*(v/c))+1.06)-18.0*(1-1.8*(v/c))),15)
