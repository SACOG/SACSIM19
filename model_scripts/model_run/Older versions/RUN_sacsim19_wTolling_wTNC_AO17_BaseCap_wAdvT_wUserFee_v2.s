    ...
    loop p=1,9                          ;endloop at line 843
	  ...
	  ; TODO: Make USECLASS period-specific in network.
         ...
         RUN PGM=HIGHWAY  MSG='step 8 Highway skims'
		  ...
		  lookup[1]=2, result=4 ;(RAD ID,Fee Factor default 1	
		  ;FAIL=1,1,1
		  ...
		  offpeak_discountfactor = 1.00 ;for no factor default set to 1.00
		  ...
		  if ((@p@=4) | (@p@=8) | (@p@=9))
			Link_UserFee = UserFeePerMile * FeeFactor * offpeak_discountfactor
		  else
		    Link_UserFee = FeeFactor * UserFeePerMile
		  endif
