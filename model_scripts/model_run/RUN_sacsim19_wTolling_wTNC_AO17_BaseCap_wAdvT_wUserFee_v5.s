...
; Previous Version: RUN_sacsim19_wTolling_wTNC_AO17_BaseCap_wAdvT_wUserFee_v5.s 8/11/2021
; SACOG
;======================================================================

;Set per-mile auto operating cost here
auto_cost_per_mile = 0.162
userfee_per_mile =   0.014

;auto_cost_per_mile_airport auto operating cost in cents, or auto_cost_per_mile*100
...
;loop time period; endloop at line 239
  ...
  run pgm=network    MSG='step 1 Initialize network'
	...
	;segment tolls from the tolls.csv file is read here and apportioned to each link in the segments
		...
		IF ((TOLLID > 0 && GPID = 0) | (@AllLaneToll@ > 0 && TOLLID > 0))
...
endrun
	...
	loop iteration=1,3             ;endloop is at the end of the script line 3966
	    ...
	    iter.s=1
            iter.m=1			
	    iter.ssi=2  ;Feedback step size = 0.5 normally
	    iter.pnrss=1
    	    iter.relgap = 0.0002
		;Iteration-specific parameters
	    if (iter.i=1)
	        iter.ssi = 1    ;always 1 for first iteration
    		iter.relgap = 0.0006  ;wider gap for early feedback iterations
    ...
    loop p=1,9                          ;endloop at line 843
	  ...
	  ; TODO: Make USECLASS period-specific in network.
         ...
         RUN PGM=HIGHWAY  MSG='step 8 Highway skims'
		  ...
		  LOOKUPI[1] = RAD_UserFee.csv			  ; User Fee Spatial Factor

          MATO="skim.auto.@per@.@votcl@.mat", MO=1-12,dec=9*2,    ; output skim matrices
              name=dati@per@_@votcl@,  tollda@per@_@votcl@, dadi@per@_@votcl@, s2ti@per@_@votcl@, tolls2@per@_@votcl@, s2di@per@_@votcl@,
                  s3ti@per@_@votcl@,  tolls3@per@_@votcl@, s3di@per@_@votcl@, daac@per@_@votcl@, s2ac@per@_@votcl@, s3ac@per@_@votcl@
          ...
          PHASE=LINKREAD                        ;define link groups
		  ;Set up User Fee Inputs
		  lookup lookupi=1, name=RAD_UserFee,
		  lookup[1]=1, result=2, ;(RAD ID,Fee Factor default 1)	
		  lookup[2]=1, result=3, ;(RAD ID,Peak Period Adjust, default is 0.00)
		  lookup[3]=1, result=4 ;(RAD ID,Off Peak Period Adjust, default is 0.00)
          ...
          CostPerMile = @auto_cost_per_mile@
		  ...
		  UserFeePerMile = @userfee_per_mile@		  
		  FeeID = li.RAD
		  FeeFactor = RAD_UserFee(1,FeeID)
		  PeakAdj = RAD_UserFee(2,FeeID)
		  OffPeakAdj = RAD_UserFee(3,FeeID)
		  
		  if ((@p@=4) | (@p@=8) | (@p@=9))
			Link_UserFee = (UserFeePerMile * FeeFactor) + OffPeakAdj
		  else
		    Link_UserFee = (UserFeePerMile * FeeFactor) + PeakAdj
		  endif
           ...
           IF (li.USECLASS == 4) ADDTOGROUP=4        ;3+ axle commercial (for off peak)
            ...
            lw.AOCost = li.distance * (CostPerMile + Link_UserFee)
			lw.cost_da = lw.AOCost + li.tollda
			lw.cost_s2 = lw.AOCost + li.tolls2
			lw.cost_s3 = lw.AOCost + li.tolls3
      ...
      if (iter.i > 1)            ;set up convergence criteria for each iteration                 
        ...
        ; Skim impedence deltas
			mw[11] = mi.5.1 + (mi.5.2*tollivot + mi.5.10*@ivot@)
			mw[12] = mi.5.4 + (mi.5.5*tollivot + mi.5.11*@ivot@) / HOV2Divisor
			mw[13] = mi.5.7 + (mi.5.8*tollivot + mi.5.12*@ivot@) / HOV3Divisor
			mw[14] = mi.9.1 + (mi.9.2*tollivot + mi.9.10*@ivot@)
			mw[15] = mi.9.4 + (mi.9.5*tollivot + mi.9.11*@ivot@) / HOV2Divisor
			mw[16] = mi.9.7 + (mi.9.8*tollivot + mi.9.12*@ivot@) / HOV3Divisor
			mw[7] = mw[14] - mw[11]
			mw[8] = mw[15] - mw[12]
			mw[9] = mw[16] - mw[13]
    ...
    IF (p = 1-5)
      run pgm=public transport msg='step 12 transit skims'
        ...
        PHASE=DATAPREP
          ...
          COST=li.distance*15,   ;*60 / 3mph to 60/4 ;3-4 adjustment to include micromobility future policy
          MAXCOST[1]=10*40.,   ;(minutes)
          ;SLACK[1]=10*5.,
          NTLEGMODE=13,
          ONEWAY=F,
          DIRECTION=3,
   ...
   *echo Begin converting trips Iter @iter.i@>timelog.beginconverttrips.iter@iter.i@.txt
    ...
    *echo Begin airport model Iter @iter.i@>timelog.beginairport.iter@iter.i@.txt
    ...
    CostPerMile = auto_cost_per_mile*100 ;needs cost per mile in cents, not dollars
    VehPerPers = 0.57
	UserFeePerMile = userfee_per_mile*100
	Link_UserFee = UserFeePerMile * 1.00 ;not apply a factor to skim impedence  
    ...
    RUN PGM=MATRIX MSG='step 30 Park-and-ride skim for airport'
        ...
        CostPerMin  = (@CostPerMile@ + @Link_UserFee@) *35/60    ;10 c/mile was MC model change by BG
        ...
        jloop
            ...
            AuCost = (AuDist*(@CostPerMile@ + @Link_UserFee@)+ zi.2.prkcost/2)*TDVehPerPers   ;per minute, was per mile
     ...
     RUN PGM=MATRIX  MSG= 'step 47 create vehicle trips matrices' 
	  ; Update successive-average vehicle-trip matrices
			...
			ofac = 1 - nfac
             ...
             jloop
                 @tolls.code1@ mw[1] = mi.2.1 * 0.1 + mi.1.1
                 @tolls.code1@ mw[2] = mi.2.2 * 0.1 + mi.1.2
                 @tolls.code1@ mw[3] = mi.2.3 * 0.1 + mi.1.3
                 
                 @tolls.code2@ mw[6] = mi.2.1*0.3 + mi.1.4
                 @tolls.code2@ mw[7] = mi.2.2*0.3 + mi.1.5
                 @tolls.code2@ mw[8] = mi.2.3*0.3 + mi.1.6
                 
				  @tolls.code3@ mw[11] = mi.2.1*0.6 + mi.1.7
                 @tolls.code3@ mw[12] = mi.2.2*0.6 + mi.1.8
                 @tolls.code3@ mw[13] = mi.2.3*0.6 + mi.1.9
	...
	loop toop = 1,5    ;toll_loop = toop
      ...
      endrun
		...
		PHASE=ILOOP
		  ...
		  PATHLOAD PATH=lw.imped1_da EXCLUDEGRP=2,3   VOL[01]=mi.1.da1,VOL[02]=mi.1.c21  

		  PATH=lw.imped1_c3 EXCLUDEGRP=2,3,4         VOL[03]=mi.1.c31

			PATH=lw.imped1_s2 EXCLUDEGRP=3   		VOL[04]=mi.1.s21 
			
			PATH=lw.imped1_s3   					VOL[05]=mi.1.s31 

			PATH=lw.imped2_da EXCLUDEGRP=2,3   		VOL[06]=mi.1.da2,VOL[07]=mi.1.c22      
			
		  PATH=lw.imped2_c3 EXCLUDEGRP=2,3,4         VOL[08]=mi.1.c32

			PATH=lw.imped2_s2 EXCLUDEGRP=3   		VOL[09]=mi.1.s22  
			
			PATH=lw.imped2_s3	   					VOL[10]=mi.1.s32
			
			PATH=lw.imped3_da EXCLUDEGRP=2,3   		VOL[11]=mi.1.da3, VOL[12]=mi.1.c23   
 
		  PATH=lw.imped3_c3 EXCLUDEGRP=2,3,4         VOL[13]=mi.1.c33
		...
		IF (p = 1-5)
			run pgm=public transport msg='transit assignment'
				...
				PHASE=DATAPREP
					...
					GENERATE,
					fromnode=31-1999, tonode=2000-20000,
					COST=li.distance*15,   ;*60 / 3mph to 60/4mph
					MAXCOST[1]=10*40.,   ;(minutes)
					;SLACK[1]=10*5.,
					NTLEGMODE=13,
					ONEWAY=F,
