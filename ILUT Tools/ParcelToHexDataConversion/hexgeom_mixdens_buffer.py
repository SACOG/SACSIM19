...
def get_buffered_aggs(fc_ilut_parcels, fc_hexs, fld_hexid, fld_pcl_id, fldin):
    
    buff_dist_ft = 1320
    
    fldout = "{}_SUM_buff".format(fldin)
    ...
    for hexid in list_hex_ids: #might be able to calculate multiple value fields at a time?
        ...
        aggval = 0
        with arcpy.da.SearchCursor(fl_parcels, fldin) as cur:
            for row in cur:
                out_val = row[0]
                aggval += out_val
        
        dict_ids_values[hexid] = aggval
...
if __name__ == '__main__':
    ...
    dict_fld_pcl_vals = {'HH_hh':['HH', agg_sum],
                         'EMPTOT':['EMP', agg_sum],
                         }
